...
function ModBenchmarks() {
  ...
  const LIGA_MAP={"cl Chile":{"Primera División":"Primera División de Chile","Primera B":"Primera B de Chile"},"AR Argentina":{"Liga Profesional":"Liga Profesional Argentina"},"BR Brasil":{"Série A":"Brasileirao Serie A"},"CO Colombia":{"Primera A":"Liga BetPlay Dimayor"},"UV Uruguay":{"Primera División":"Campeonato Uruguayo"},"PE Perú":{"Liga 1":"Liga 1 Perú"},"EC Ecuador":{"LigaPro Serie A":"LigaPro Ecuador"},"MX México":{"Liga MX":"Liga MX"},"US MLS":{"MLS":"MLS"},"SA Saudi":{"Pro League":"Saudi Pro League"},"JP Japón":{"J1 League":"J1 League"}};
  const ligaNombre=LIGA_MAP[pais]?.[div]||div;
  ...
  const jugadoresLiga=useMemo(()=>{if(!dbPro)return[];return dbPro.jugadores.filter(j=>{const ml=j.l&&(j.l===ligaNombre||j.l===div||(j.l||"").toLowerCase().includes(div.toLowerCase().substring(0,8)));const mc=equipo==="__todos__"||j.eq===equipo;return ml&&mc&&posAC(j.pos)===posAC(pos)&&j.s?.rat;});},[dbPro,pais,div,equipo,pos,ligaNombre]);
  const todosLiga=useMemo(()=>{if(!dbPro)return[];return dbPro.jugadores.filter(j=>{const ml=j.l&&(j.l===ligaNombre||j.l===div||(j.l||"").toLowerCase().includes(div.toLowerCase().substring(0,8)));return ml&&(equipo==="__todos__"||j.eq===equipo)&&j.s?.rat;}).sort((a,b)=>(b.s?.rat||0)-(a.s?.rat||0));},[dbPro,pais,div,equipo,ligaNombre]);
